...
server.post("/practices/", function(req, res, next) {
  const error = validatePractice(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new practice.
    next();
  }
});

server.post("/consultants/", function(req, res, next) {
  const error = validateGP(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createGPSlug(req.body.title); // Generate a slug for new GP
    next();
  }
});
...
function validatePractice(practice) {
  if (!practice.code) return "Code is required.";
  return "";
}

function validateGP(consulant) {
  if (!consultant.name) return "Name is required.";
  if (!consultant.gmc) return "GMC Code is required.";
  return "";
}
